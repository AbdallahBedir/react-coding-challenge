  ...
  const handleSearchClick = (q) => {
    if (q && typeof q === "string" && q.length >= 3) {
      ...
      if (history.indexOf(q) === -1) {
        // To remove duplicate items in history
  ...
  return (
    <React.Fragment>
      <MuiAutocomplete
        ...
        options={history}
        renderOption={(
          option // Display history search item & close icon
        ) => (
          <div className={classes.autocompleteOption}>
            <div>{option}</div>
            <IconButton
              onClick={(e) => removeItem(e, option)}
              type="submit"
              aria-label="close"
            >
              <CloseIcon />
              ...
              InputProps={{
                ...
                endAdornment: (
                  <InputAdornment position="end">
                    <IconButton
                      disabled={
                        // Disable submit button if value < 3 letters
                        (params &&
                          params.inputProps &&
